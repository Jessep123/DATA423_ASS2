...
# ================================================================================
#       GLOBAL PLOT STYLING HELPERS
# ================================================================================
  
  eda_plot_theme <- function() {
    theme_minimal() +
      theme(
  ...
  set_base_plot_style <- function() {
    par(
      cex.main = 20 / 12,   # approx size 20
      font.main = 2,        # bold
      cex.sub  = 16 / 12,   # approx size 16
      font.sub = 1          # plain
    )
  }
...
# ================================================================================
  ...
  reactive_dataset <- function(label) {
      ...
      reactive({
        if (label == "missing_processing"){
        if (input$selected_data_EDA == "Base Data"){df <- data}
        else if (input$selected_data_EDA == "Train Split"){df <- train_data}
        else if (input$selected_data_EDA == "Test Split"){df <- test_data}
        }
        #For reactive datasets in Exploratory Data Analysis section, use unadjusted dataset
        else if (label != "missing_processing"){df <- data_og}
        
        #For reactive datasets in Exploratory Data Analysis section, use unadjusted dataset
        # if (label != "missing_processing"){df <- data_og}
...
# ================================================================================
    ...
    processed_eda_reactive <- reactive({
      ...
      for (outlier_step in outlier_steps) {
        method <- outlier_step$method
        cols <- intersect(outlier_step$cols, names(df))
        criteria <- outlier_step$criteria
        threshold <- outlier_step$threshold
        ...
        for (col in cols) {
          ...
          if (length(unique(stats::na.omit(vals))) <= 2) next
          
          if (criteria == "Standard Deviation (Z-Score)") {
          ...
          } else if (criteria == "Inter-Quartile Range") {
            iqr_val <- IQR(vals, na.rm = TRUE)
            if (is.na(iqr_val) || iqr_val == 0) next
            
            q1 <- quantile(vals, 0.25, na.rm = TRUE, names = FALSE)
            q3 <- quantile(vals, 0.75, na.rm = TRUE, names = FALSE)
          ...
          if (method == "Winsorize") {
            df[[col]] <- ifelse(
              is.na(df[[col]]),
              NA,
              pmin(pmax(df[[col]], lower), upper)
            )
          } else if (method == "Replace with NA") {
            df[[col]] <- ifelse(
              !is.na(df[[col]]) & (df[[col]] < lower | df[[col]] > upper),
              NA,
              df[[col]]
            )
          }
    ...
    #List of column-wise outlier processing settings
    outlier_processing_settings <- reactiveVal(list())
    
    observeEvent(input$apply_outlier_processing, {
      current_steps <- outlier_processing_settings()
      
      method <- input$outlier_processing_method
      cols <- input$selected_vars_outlier_processing_apply
      criteria <- input$outlier_processing_criteria
      
      threshold <- NULL
      
      if (criteria == "Standard Deviation (Z-Score)") {
        threshold <- input$outlier_processing_sd_threshold
      } else if (criteria == "Inter-Quartile Range") {
        threshold <- input$outlier_processing_iqr_threshold
      }
      ...
      outlier_processing_settings(append(current_steps, list(new_step)))
    })
    
    #Reset processing settings
    observeEvent(input$reset_outlier_processing_steps, {
      outlier_processing_settings(list())
    })
    
    #Update selection columns for outlier processing
    observe({
      df <- processed_eda_reactive()
  ...
  # ================================================================================
   ...
   output$summary_table <- renderUI({
    ...
    output$variable_summary <- renderUI({
      layout_columns(
        value_box("Rows", nrow(data_og)),
        value_box("Variables", ncol(data_og)),
        value_box("Numeric Variables", sum(sapply(data_og, is.numeric))),
        value_box("Categorical Variables", sum(sapply(data_og, is.factor))),
        value_box("Date Variables", sum(sapply(data_og, is.Date))),
...
# ================================================================================
    ...
    output$missing_data <- renderPlot({
      ...
      if (input$selected_data_EDA == "Base"){data <- data}
      else if (input$selected_data_EDA == "Train Split"){data <- train_data}
      else if (input$selected_data_EDA == "Test Split"){data <- test_data}
...
# =================================================================================
    ...
    if (input$selected_data_EDA == "Base Data"){og_data <- data}
    else if (input$selected_data_EDA == "Train Split"){og_data <- train_data}
    else if (input$selected_data_EDA == "Test Split"){og_data <- test_data}
    
    paste0(
      row_count, " / ", nrow(og_data), " Rows | ",
    ...
    output$missing_data_processing <- renderPlot({
      ...
      print(round(missing_pct["VAX_RATE"], 0))
      print(names(df))
      #Setting colour/legend inputs depending on if distinct datatypes selected or not
...
# =================================================================================
   #Density plot 
    ...
    output$boxplot_outlier <- renderPlot({
      ...
      if (is.null(num_var) || length(num_var) == 0) {
        num_var <- names(df[, sapply(df, is.numeric), drop = FALSE])
      }
      
      # ensure numeric only
      num_var <- num_var[num_var %in% names(df)[sapply(df, is.numeric)]]
      ...
      cols_to_keep <- c("CODE", num_var, colour_var)
      cols_to_keep <- cols_to_keep[cols_to_keep %in% names(df)]
      
      df <- df %>% select(all_of(cols_to_keep))
      
      req(length(num_var) > 0)
      
      iqr_multiplier <- input$iqr_outlier
      
      # ---- Axis label ----
      if (isTRUE(input$center_data_outlier) && isTRUE(input$scale_data_outlier)) {
        x_label <- "Standardised Values (Z-Score)"
      } else if (isTRUE(input$center_data_outlier) && !isTRUE(input$scale_data_outlier)) {
        x_label <- "Centered Values"
      } else if (!isTRUE(input$center_data_outlier) && isTRUE(input$scale_data_outlier)) {
        x_label <- "Scaled Values"
      } else {
        x_label <- "Unadjusted Values"
      }
      
      # ---- Title logic ----
      total_numeric_vars <- 11
      
      if (length(num_var) == total_numeric_vars) {
        plot_title <- "Boxplot of All Variables"
      } else {
        plot_title <- paste("Boxplot of", paste(num_var, collapse = " and "))
      }
      
      if (length(num_var) == 1 && !is.null(colour_var) && colour_var %in% names(df)) {
        plot_title <- paste0(plot_title, " Split by ", colour_var)
      }
      
      # ---- CASE 1: Single variable split by categorical variable ----
      if (length(num_var) == 1 && !is.null(colour_var) && colour_var %in% names(df)) {
        ...
        p <- ggplot(df_long, aes(x = Value, y = Group)) +
          geom_boxplot(fill = "#4C78A8", alpha = 0.6, outlier.shape = NA) +
          geom_point(
            data = outliers,
            aes(x = Value, y = Group),
            color = "red",
        ...
        if (isTRUE(input$iqr_outlier_label) && nrow(outliers) > 0) {
          p <- p +
            ggrepel::geom_text_repel(
              data = outliers,
              aes(x = Value, y = Group, label = CODE),
              direction = "both",
              hjust = 0,
              size = 3,
              box.padding = 0.25,
        ...
        # ---- CASE 2: One or more numeric variables, no grouping ----
        df_long <- df %>%
          tidyr::pivot_longer(
            cols = all_of(num_var),
            names_to = "Variable",
        ...
        if (isTRUE(input$iqr_outlier_label) && nrow(outliers) > 0) {
          p <- p +
            ggrepel::geom_text_repel(
              data = outliers,
              aes(x = Value, y = Variable, label = CODE),
              direction = "both",
              hjust = 0,
              size = 3,
              box.padding = 0.25,
    ...
    output$outlier_scatter <- renderPlot({
      ...
      if (mark_outlier_flag){
        ...
        if (outlier_method == "Inter-Quartile Range"){
          ...
          outlier_info <- paste0("Outlier Criteria = Interquartile Range | IQR Multiplier = ", iqr_multiplier)
          ...
          outlier_info <- paste0("Outlier Criteria = Standard Deviations From Mean | Threshold = ", input$outlier_scatter_sd_num)
        ...
        #Otherwise outlier_info is null
      } else{outlier_info <- NULL}
      ...
      if (input$univariate_facet != "None"){
        ...
        outlier_info <- paste0("Facet Wrap With ", input$univariate_facet, "\n", outlier_info)
      }
    ...
    output$outlier_scatter_bivariate <- renderPlot({
 
    df <- processed_eda_reactive()
    x_var <- input$outlier_scatter_bivariate_x_var
    x_var_data <-  df[[x_var]]
    y_var <- input$outlier_scatter_bivariate_y_var
    y_var_data <-  df[[y_var]]
    ...
    #Plotting if bagplot is selected
    if (outlier_method == "Bagplot") {
      bagplot_comp <- aplpack::compute.bagplot(
        x = df[[x_var]],
        y = df[[y_var]],
        factor = input$outlier_bivariate_scatter_bag_inflation
      )
      ...
      aplpack::bagplot(
        ...
        y = df[[y_var]],
        factor = input$outlier_bivariate_scatter_bag_inflation,
        main = paste("Bagplot of", y_var, "vs", x_var),
        xlab = x_var,
        ylab = y_var
      )
      ...
      #Adding outlier labels if it is ticked
      if (isTRUE(input$outlier_bivariate_scatter_mark) && length(bagplot_comp$pxy.outlier) > 0) {
        
        outlier_pts <- as.data.frame(bagplot_comp$pxy.outlier)
        names(outlier_pts) <- c("x", "y")
        
        #Getting code label from dataframe to label outliers 
        label_df <- dplyr::inner_join(
          df %>%
            dplyr::mutate(
              x = .data[[x_var]],
              y = .data[[y_var]]
    ...
    if (outlier_method == "Mahalanobis Distance") {
      
      #Plotting dataframe without code for mahalanobis calculation
      maha_calc_df <- df[, c(x_var, y_var), drop = FALSE]
      maha <- mahalanobis(
        x = maha_calc_df,
        center = colMeans(maha_calc_df),
        cov = cov(maha_calc_df)
      )
      
      maha_threshold <-  qchisq(input$outlier_bivariate_scatter_mahalanobis, df = 2)
      
      #Calculating outliers based on mahalanobis 
      df$outlier_flag <- ifelse(maha > maha_threshold, "Outlier", "Non-Outlier")
      
      #Adding label for outliers
      ...
      plot <- ggplot(df, 
        ...
        geom_point(aes(colour = outlier_flag)) +
        labs(
          title = paste("Mahalanobis Outliers:", y_var, "vs", x_var),
          subtitle = paste("Chi-square quantile =", input$outlier_bivariate_scatter_mahalanobis),
          x = x_var,
          y = y_var
        ) +
        eda_plot_theme()
...
# ===================================================================================
    ...
    observeEvent(input$add_processing_step, {
      ...
      #Outlier criteria is null
      #WIll be changed if replace outlier with NA is selected
      ...
      if (input[[input_method]] == "impute_knn"){
      ...
      else if (input[[input_method]] == "replace_outliers"){
        criteria <- input[[paste0("step", id, "columnwise_outlier_criteria")]]

        if (criteria == "Standard Deviation (Z-Score)"){
          threshold_input <- paste0("step", id, "column_wise_outlier_sd")
          additional_info <- isolate(input[[threshold_input]])
        }
        else if(criteria == "Inter-Quartile Range"){
          threshold_input <- paste0("step", id, "column_wise_outlier_iqr_mult")
          additional_info <- isolate(input[[threshold_input]])
        }
      }
      else if (input[[input_method]] == "winsorize"){
        additional_info <- input[[paste0("step", id, "winsorize_range")]]
      }
      else if (input[[input_method]] == "remove_outliers"){
        criteria <- input[[paste0("step_", id, "remove_criteria")]]
        
        if (criteria == "Local Outlier Factor"){
          min_pts <- input[[paste0("step_", id, "_lof_minpts")]]
          threshold <- input[[paste0("step_", id, "_lof_threshold")]]
          additional_info <- c(min_pts, threshold)
        }
        else if (criteria == "Isolation Forest"){
         iso_threshold <- input[[paste0("step_", id, "_iso_threshold")]]
         num_trees <- input[[paste0("step_", id, "_iso_ntrees")]]
         include_cats <- input[[paste0("step_", id, "_iso_trees_include_cats")]]
         additional_info <- c(iso_threshold, num_trees, include_cats)
        }
        else if (criteria == "Mahalanobis Distance"){
           chi_thresh <- input[[paste0("step_", id, "_mahal_threshold")]]
           robust_flag <- input[[paste0("step_", id, "_mahal_robust")]]
        ...
        additional_info <- isolate(input[[paste0("step_interact_keep_og_", id)]])
        if (length(cols) != 2) {
          showNotification(
      ...
      else if (input[[input_method]] == "shadow_var"){
        operation <- isolate(input[[paste0("step_shadow_operation", id)]])
        value <- isolate(input[[paste0("step_shadow_num_", id)]])
        cols <- isolate(input[[paste0("step_col_shadow_", id)]])
        additional_info <- list(
          operation = operation,
          value = value
        )
      }
      else if (input[[input_method]] == "remove_var"){
        cols <- isolate(input[[paste0("step_col_remove_", id)]])
        additional_info <- 0
      }
      ...
      card(
        ...
        selectInput(
          ...
          choices  = c(
            ...
            "Replace Outliers with NA"     = "replace_outliers",
            "Winsorize Outliers"     = "winsorize",
            "Add Interaction Term"     = "interaction_term",
            "Create Shadow Variable" = "shadow_var"
            
            
          ),
          selected = ""
        ),            
        # Conditional panel for removing variable
        conditionalPanel(
          condition = paste0("input['step_method_", step_id, "'] == 'remove_var'"),
          pickerInput(
            inputId  = paste0("step_col_remove_", step_id),
        ...
        # Conditional panel for creating shadow variable
        conditionalPanel(
          condition = paste0("input['step_method_", step_id, "'] == 'shadow_var'"),
          pickerInput(
            inputId  = paste0("step_col_shadow_", step_id),
          ...
          pickerInput(
            inputId  = paste0("step_shadow_operation", step_id),
            label    = "Operation",
            choices  = c(">", "<", "=", "is na"),
            selected = ">",
            multiple = FALSE,
          ...
          numericInput(
            inputId  = paste0("step_shadow_num_", step_id),
            label = "Value - Doesn't Matter if Operation 'is na'",
            value = 500
          )
        ),
        ...
        conditionalPanel(
          ...
          sliderInput(
            ...
            min = 0, max = ncol(df), value = 3, step = 1
          ),
          checkboxInput(
        ...
        conditionalPanel(
          ...
          pickerInput(
            inputId  = paste0("step_", step_id, "remove_criteria"),
            label    = "Method",
            choices  = c("Mahalanobis Distance", "Isolation Forest", "Local Outlier Factor"),  
            selected = NULL,
            multiple = FALSE,
            options  = list(`actions-box` = TRUE, `live-search` = TRUE)
          ),
          p("Mahalanobis and Local Outlier Factor are vulnerable to variable scale so are best when data has been standardized first",
            style = "color:red;"),
          conditionalPanel(
            condition = paste0("input['step_", step_id, "remove_criteria'] == 'Mahalanobis Distance'"),
            sliderInput(
              paste0("step_", step_id, "_mahal_threshold"),
              "Chi-Square Quantile Threshold",
              min = 0.90,
              max = 0.999,
              value = 0.975,
              step = 0.001
            ...
            checkboxInput(
              paste0("step_", step_id, "_mahal_robust"),
              "Use Robust Covariance",
              value = FALSE
            )
          ),
          conditionalPanel(
            condition = paste0("input['step_", step_id, "remove_criteria'] == 'Isolation Forest'"),
            numericInput(
              paste0("step_", step_id, "_iso_threshold"),
            ...
            numericInput(
              paste0("step_", step_id, "_iso_ntrees"),
              "Number of Trees",
              value = 100,
              min = 10,
              step = 10
            ...
            checkboxInput(
              paste0("step_", step_id, "_iso_trees_include_cats"),
              "Include Binary / Dummy-Encoded Variables",
              value = FALSE
            )
          ),
          conditionalPanel(
            condition = paste0("input['step_", step_id, "remove_criteria'] == 'Local Outlier Factor'"),
            numericInput(
              paste0("step_", step_id, "_lof_minpts"),
              "Minimum Points",
              value = 5,
              min = 1,
              step = 1
            ...
            numericInput(
              paste0("step_", step_id, "_lof_threshold"),
              "LOF Outlier Threshold",
              value = 2,
              min = 1,
              step = 0.1
            )
        ...
        # Column wise outlier methods
        conditionalPanel(
          condition = paste0( "input['step_method_", step_id, "'] == 'replace_outliers'"),
          pickerInput(paste0("step", step_id, "columnwise_outlier_criteria"),
                      "Outlier Classification Criteria",
                      choices = c("Standard Deviation (Z-Score)", "Inter-Quartile Range"),
                      selected = "Standard Deviation (Z-Score)"),
          conditionalPanel(
            condition = paste0("input.step", step_id, "columnwise_outlier_criteria == 'Standard Deviation (Z-Score)'"),
            numericInput(
              paste0("step", step_id, "column_wise_outlier_sd"),
              "Standard Deviations",
              value = 3
            )
          ),
          conditionalPanel(
            condition = paste0("input.step", step_id, "columnwise_outlier_criteria == 'Inter-Quartile Range'"),
            numericInput(
              paste0("step", step_id, "column_wise_outlier_iqr_mult"),
              "Inter-Quartile Range Multiplier",
              value = 1.5
            )
          ),
          p("All calculations are done column wise")
          
        ),
        
        #Winsorizing range
        conditionalPanel(
            condition = paste0( "input['step_method_", step_id, "'] == 'winsorize'" ),
          sliderInput(
            paste0("step", step_id, "winsorize_range"),
            "Winsorize Percentile Range",
            min = 0,
            max = 100,
            value = c(5, 95),
            post = '%'
      ...
      tagList(
        lapply(seq_along(steps), function(i) {
          ...
          if (method == "remove_na_rows"){
            ...
            contextual_info <- paste0("| ", criteria, " = ", val)
          } 
            else  if (method == "winsorize"){
              range_lower <- step$additional_info[1]
              range_upper <- step$additional_info[2]
              
              contextual_info <- paste0("| Percentile Range ", range_lower, "% - ", range_upper, "%")
            }
          else  if (method == "remove_outliers"){
            details <- step$additional_info
            criteria <- step$outlier_criteria
            if (criteria == "Local Outlier Factor"){
              contextual_info  <- paste0(" | ", criteria, " | Min Points = ", details[1], " | Outlier Threshold = ", details[2])
            }
            else if (criteria == "Isolation Forest"){
              if (details[[3]] == 1){include_cat <-paste0(" | Dummy Encoded Vars Included")}
              else if (details[[3]] != 1){include_cat <- NULL}
              contextual_info  <- paste0(" | ", criteria, " | Outlier Threshold = ", details[1], " | Num Trees = ", details[2], include_cat)
            }
            else if (criteria == "Mahalanobis Distance"){
              # if (isTRUE(details[[2]])){rob <-paste0(" | Robust Covariance Measure")}
              if (details[[2]] == 1){rob <-paste0(" | Robust Covariance Measure")}
          ...
          else if (method == "shadow_var"){
            op <- step$additional_info$operation
            val <- step$additional_info$value
            if (op != "is na"){
            contextual_info <- paste0("| ", cols[1], " ", op, " ", val)
            }
            else {contextual_info <- paste0("| ", cols[1], " ", op)}
          } 
          else if (method == "remove_var"){
            contextual_info <- paste0("| Remove: ", paste(cols, collapse = ", "))
          } 
    ...
    saved_pipelines <- reactiveVal(list(
      ...
      "Adjusted Pipeline" = list(
        recipe  = adjusted_recipe,
        df      = juice(adjusted_prepped),
    ...
    pipeline_recipe <- eventReactive(input$process_data_btn, {
      ...
      #In case cat vars are removed using step_rm()
      removed_cats <- c()
      ...
      for (i in seq_along(steps)) {
        ...
        if (method == "remove_na_rows") {
          ...
          lincomb_cols <- intersect(
            names(current_data),
            setdiff(names(non_cat_vars), "DEATH_RATE")
          )
          
          pipeline_recipe <- pipeline_recipe %>% step_lincomb(all_of(!!lincomb_cols))
        }
        else if (method == "winsorize") {
          ...
          winsorize_range <- step$additional_info / 100
          for (col in cols) {
            col_sym <- rlang::sym(col)
            
            qs <- quantile(current_data[[col]], probs = winsorize_range, na.rm = TRUE)
            lower <- qs[1]
            upper <- qs[2]
          ...
          for (col in cols) {
            ...
            if (criteria == "Standard Deviation (Z-Score)") {
              ...
              if (is.na(s) || s == 0) next
              m <- mean(vals, na.rm = TRUE)
              lower <- unname(m - step$additional_info * s)
              upper <- unname(m + step$additional_info * s)
            } else if (criteria == "Inter-Quartile Range") {
              iqr_val <- IQR(vals, na.rm = TRUE)
              if (is.na(iqr_val) || iqr_val == 0) next
              q1 <- quantile(vals, 0.25, na.rm = TRUE, names = FALSE)
              q3 <- quantile(vals, 0.75, na.rm = TRUE, names = FALSE)
              lower <- unname(q1 - step$additional_info * iqr_val)
              upper <- unname(q3 + step$additional_info * iqr_val)
            } 
            ...
            pipeline_recipe <- pipeline_recipe %>%
              step_mutate(
                !!col_sym := ifelse(
                  is.na(!!col_sym),
                  NA_real_,
                  ifelse(!!col_sym < !!lower | !!col_sym > !!upper, NA_real_, !!col_sym)
                )
              )
          }
        }
          ...
          prepped <- prep(pipeline_recipe, training = df)
          current_data <- juice(prepped)
          
          # Keep CODE for mapping back
          codes <- current_data$CODE
          
          # Default matrix for Mahalanobis + LOF
          use_cols <- intersect(
            names(current_data),
            setdiff(names(non_cat_vars), "DEATH_RATE")
          )
          
          x <- current_data[, use_cols, drop = FALSE]
          x <- x[, sapply(x, is.numeric), drop = FALSE]
          x <- x[, sapply(x, function(z) {
            s <- sd(z, na.rm = TRUE)
            !is.na(s) && s > 0
          }), drop = FALSE]
          ...
          if (criteria == "Local Outlier Factor"){
            min_pts   <- as.numeric(details[1])
            threshold <- as.numeric(details[2])
            
            if (nrow(x) <= min_pts || ncol(x) < 1) {
              showNotification(
                paste0("Step ", i, " LOF skipped: not enough usable rows/columns."),
                type = "warning"
              )
              next
            }
            
            lof_scores <- dbscan::lof(as.matrix(x), minPts = min_pts)
            keep_codes <- codes_cc[lof_scores <= threshold]
            
          } else if (criteria == "Isolation Forest"){
            iso_threshold <- as.numeric(details[1])
            num_trees     <- as.numeric(details[2])
            include_dummy <- as.logical(as.numeric(details[3]))
            
            if (include_dummy) {
              use_cols <- setdiff(names(current_data), c("CODE", "DEATH_RATE", "OBS_TYPE"))
              x <- current_data[, use_cols, drop = FALSE]
              x <- x[, sapply(x, is.numeric), drop = FALSE]
            } else {
              use_cols <- intersect(
            ...
            if (nrow(x) < 5 || ncol(x) < 1) {
              showNotification(
                paste0("Step ", i, " Isolation Forest skipped: not enough usable rows/columns."),
                type = "warning"
              )
              next
            }
            ...
            iso_scores <- predict(iso_model, x)
            keep_codes <- codes_cc[iso_scores <= iso_threshold]
            
          } else if (criteria == "Mahalanobis Distance"){
            chi_prob <- as.numeric(details[1])
            robust   <- as.logical(as.numeric(details[2]))
            
            if (nrow(x) < 5 || ncol(x) < 2) {
              showNotification(
                paste0("Step ", i, " Mahalanobis skipped: not enough usable rows/columns."),
                type = "warning"
              )
              next
            }
            ...
            inv_ok <- !is.null(tryCatch(qr.solve(covmat), error = function(e) NULL))
            if (!inv_ok) {
              showNotification(
                paste0("Step ", i, " Mahalanobis skipped: covariance matrix not invertible."),
                type = "warning"
              )
              next
            }
            
            maha <- mahalanobis(x, center = center, cov = covmat)
            threshold <- qchisq(chi_prob, df = ncol(x))
            keep_codes <- codes_cc[maha <= threshold]
            
            print(paste("Mahalanobis rows used:", nrow(x)))
            print(paste("Rows removed:", sum(maha > threshold)))
          }
          ...
          if (var1_is_num && var2_is_num) {
            ...
            new_name <- paste(var1, var2, sep = "_x_")
            
            if (new_name %in% c(names(df), created_cols)) {
              showNotification(
                paste0("Interaction term '", new_name, "' already exists."),
                type = "error"
              )
              next
            }
          ...
          source_col <- cols[1]
          source_sym <- rlang::sym(source_col)
          
          op <- step$additional_info$operation
          val <- as.numeric(step$additional_info$value)
          
          new_col <- paste0(source_col, "_shadow")
          new_col_sym <- rlang::sym(new_col)
          
          if (op == ">") {
            pipeline_recipe <- pipeline_recipe %>%
              step_mutate(
                !!new_col_sym := dplyr::if_else(
                  is.na(!!source_sym),
                  NA_real_,
                  ifelse(!!source_sym > val, 1, 0)
                )
              )
            ...
            na_col <- paste0(source_col, "_missing")
            na_col_sym <- rlang::sym(na_col)
            
            pipeline_recipe <- pipeline_recipe %>%
              step_mutate(
                !!na_col_sym := ifelse(is.na(!!source_sym), 1, 0) 
              ) 
          }
        }
        else if (method == "remove_var"){
          pipeline_recipe <- pipeline_recipe %>% step_rm(all_of(!!cols))
          
          removed_cats <- c(
            removed_cats,
            intersect(cols, c("GOVERN_TYPE", "HEALTHCARE_BASIS"))
            )
      ...
      remaining_nominals <- setdiff(
        ...
        c("CODE", "OBS_TYPE", interaction_encoded_cats, removed_cats)
      )
...
# ====================================================================================
    ...
    reactive_model <- reactive({
      ...
      train_y_original <- train_processed$DEATH_RATE
      test_y_original  <- test_processed$DEATH_RATE
      
      if (isTRUE(input$log_transform_outcome)) {
        train_processed$DEATH_RATE <- log1p(train_processed$DEATH_RATE)
        test_processed$DEATH_RATE  <- log1p(test_processed$DEATH_RATE)
      }
      
      train_processed$DEATH_RATE_original <- train_y_original
      test_processed$DEATH_RATE_original  <- test_y_original
      
      case_weights <- NULL
      if (isTRUE(input$weight_extremes)) {
        threshold <- quantile(
          train_y_original,
          probs = input$extreme_quantile,
          na.rm = TRUE
        )
        
        case_weights <- ifelse(
          train_y_original >= threshold,
          input$extreme_weight,
          1
        )
      }
      ...
      model <- caret::train(
        DEATH_RATE ~ . - CODE - OBS_TYPE - DEATH_RATE_original,
      ...
      if (isTRUE(input$log_transform_outcome)) {
        test_preds_eval <- expm1(test_preds)
        test_actual_eval <- test_y_original
      } else {
        test_preds_eval <- test_preds
        test_actual_eval <- test_y_original
      }
      
      test_performance <- postResample(
        pred = test_preds_eval,
        obs  = test_actual_eval
      )
      
      best_alpha  <- model$bestTune$alpha
      best_lambda <- model$bestTune$lambda
      
      train_cv_preds <- model$pred %>%
        dplyr::filter(alpha == best_alpha, lambda == best_lambda)
      
      if (isTRUE(input$log_transform_outcome)) {
        train_cv_preds <- train_cv_preds %>%
          dplyr::mutate(
            pred_eval = expm1(pred),
            obs_eval  = expm1(obs)
          )
      } else {
        train_cv_preds <- train_cv_preds %>%
      ...
      train_processed <- train_processed %>%
        ...
        dplyr::left_join(
          train_cv_preds %>% dplyr::select(rowIndex, pred_eval, obs_eval),
          by = "rowIndex"
        ) %>%
        dplyr::rename(
          Pred_DeathRate = pred_eval,
          Actual_DeathRate = obs_eval
        )
      
      train_perf <- postResample(
        pred = train_processed$Pred_DeathRate,
        obs  = train_processed$Actual_DeathRate
      )
      
      test_processed <- test_processed %>%
        dplyr::mutate(
          Pred_DeathRate = test_preds_eval,
          Actual_DeathRate = test_actual_eval
        )
      
      coefs <- coef(model$finalModel, s = best_lambda)
      ...
      list(
        ...
        test_preds = test_preds_eval,
        test_actual = test_actual_eval,
        train_processed = train_processed,
        test_processed = test_processed,
    ...
    output$model_predictions <- renderPlot({
      ...
      ggplot(df_plot, aes(x = Actual, y = Pred)) +
        ...
        labs(title = paste0("Actual vs Predicted | ", input$selected_pipeline_model),
             x = "Actual", 
             y = "Predicted",
    ...
    output$model_results <- render_gt({
      ...
      test_perf <- model_list$performance
      train_perf <- model_list$train_perf
      
      #Dataframe of train vs test results
      results <- data.frame(
        Metric = names(test_perf),
        Train  = as.numeric(train_perf[names(test_perf)]),
        Test   = as.numeric(test_perf)
      )
      
      #NIce gt output for the table
      results %>%
        gt::gt() %>%
        gt::fmt_number(
        ...
        gt::tab_header(
          ...
          subtitle = "Train vs Test"
        )
      
    })
    
    output$model_details <- render_gt({
      model_list <- reactive_model()
      
      coef_df <- model_list$coef_df
      
      non_zero_coef <- sum(coef_df$Coefficient != 0) - 1  # exclude intercept
      ...
      details <- data.frame(
        ...
        Value = c(
          model_list$best_alpha,
          model_list$best_lambda,
          non_zero_coef,
          input$cv_folds,
          nrow(model_list$train_processed),
          nrow(model_list$test_processed)
        )
      )
      
      details %>%
        gt::gt() %>%
        gt::fmt_number(columns = Value,
                       rows = Statistic %in% c("Best alpha", "Best lambda"),
                       decimals = 3) %>%
        gt::fmt_number(columns = Value,
                       rows = Statistic %in% c(
                         "Non-zero coefficients",
    ...
    output$coef_plot <- renderPlotly({
      ...
      ggplotly(
        ggplot(coef_df, aes(x = reorder(Variable, abs(Coefficient)), y = Coefficient)) +
          ...
          coord_flip() +
          labs(
            title = paste0("Model Coefficients | ", input$selected_pipeline_model),
            x = NULL,
            y = "Coefficient Value"
          ) +
    ...
    output$varimp_plot <- renderPlotly({
      ...
      ggplotly(
        ggplot(imp_df, aes(x = reorder(Variable, Importance), y = Importance)) +
          ...
          coord_flip() +
          labs(title = paste0("Variable Importance | ", input$selected_pipeline_model), x = NULL, y = "Importance") +
          eda_plot_theme()
      )
    })
    ...
    output$model_outlier_plot <- renderPlot({
      ...
      if (selected_data == "Test"){
        ...
        actual <- result$test_processed$Actual_DeathRate
        preds  <- result$test_processed$Pred_DeathRate
        ...
        actual <- df$Actual_DeathRate
        preds  <- df$Pred_DeathRate
        residuals   <- actual - preds
        
      } else if (selected_data == "Both"){
        train <- result$train_processed %>%
          dplyr::transmute(
            CODE = CODE,
            Actual_DeathRate = Actual_DeathRate,
            Pred_DeathRate = Pred_DeathRate
          )
        ...
        df <- dplyr::bind_rows(train, test) %>%
          dplyr::left_join(
            data %>% dplyr::select(CODE, GOVERN_TYPE, HEALTHCARE_BASIS),
            by = "CODE"
          )
    ...
    output$outlier_density_box_plot <- renderPlot({
      ...
      resid <- actual - preds
        ...
        actual <- result$train_processed$Actual_DeathRate
        preds  <- result$train_processed$Pred_DeathRate
        ...
        plot_data <- dplyr::bind_rows(train, test)
      ...
      lower_whisker <- min(plot_df$Residual[plot_df$Residual >= lower_bound], na.rm = TRUE)
      upper_whisker <- max(plot_df$Residual[plot_df$Residual <= upper_bound], na.rm = TRUE)
      med <- median(plot_df$Residual, na.rm = TRUE)
      ...
      p_box <- ggplot() +
        geom_boxplot(data = box_df,
                     aes(x = xmin, ymin = ymin, lower = lower, middle = middle, upper = upper, ymax = ymax),
                     stat = "identity",
                     width = 0.25
        ) +
        geom_point(
    ...
    output$resid_predictor <- renderPlot({
      ...
      if (selected_data == "Test"){
        plot_data <- result$test_processed %>%
          dplyr::select(CODE, Actual_DeathRate, Pred_DeathRate, all_of(predictor)) %>%
          dplyr::mutate(
            resid = Actual_DeathRate - Pred_DeathRate
          )
      ...
      if (length(unique(plot_data[[predictor]])) != 2){
        plot <- ggplot(plot_data, aes(x = .data[[predictor]], y = resid)) +
          geom_point(aes(colour = Colour), alpha = 0.7) +
          geom_hline(yintercept = 0, linetype = "dashed") +
          eda_plot_theme()
        
        if (input$resid_predict_smooth_line){
          plot <- plot + geom_smooth(se = FALSE, colour = "#F4A6A6")
        }
        
        if (input$interaction_cat_var != "None"){
          plot <- plot +
            facet_wrap(~ .data[[input$interaction_cat_var]]) +
            theme(
              panel.border = element_rect(colour = "black", fill = NA, linewidth = 0.8)
            )
        }
        ...
        plot <- ggplot(plot_data, aes(x = as.factor(.data[[predictor]]), y = resid)) +
          geom_boxplot(outlier.shape = NA, width = 0.25, fill = "lightblue") +
          geom_point(
    ...
    get_glm_data <- function(model){
      ...
      if (selected_data == "Test") {
        data <- model$test_processed %>%
          dplyr::select(-OBS_TYPE) %>%
          dplyr::mutate(
            DEATH_RATE = Actual_DeathRate
          )
        
      } else if (selected_data == "Train") {
        data <- model$train_processed %>%
          dplyr::select(-c(OBS_TYPE, rowIndex)) %>%
          dplyr::mutate(
            DEATH_RATE = Actual_DeathRate
          )
        
      } else if (selected_data == "Both") {
        train <- model$train_processed %>%
          dplyr::select(-rowIndex) %>%
          dplyr::mutate(DEATH_RATE = Actual_DeathRate)
        
        test <- model$test_processed %>%
          dplyr::mutate(DEATH_RATE = Actual_DeathRate)
    ...
    output$glm_data_export <- downloadHandler(
      ...
      content = function(file){
        ...
        data <- data %>%
          dplyr::select(all_of(c(selected_vars, "DEATH_RATE", "Pred_DeathRate"))) %>%
          dplyr::mutate(Residual = DEATH_RATE - Pred_DeathRate) %>%
          dplyr::relocate(c(DEATH_RATE, Pred_DeathRate, Residual), .before = everything())
